...
export const ExperienceTimeline: React.FC<ExperienceTimelineProps> = ({ experiences = [] }) => {
  const [selectedExp, setSelectedExp] = useState<ExperienceTimelineItem | null>(null);
  const [activeGalleryImg, setActiveGalleryImg] = useState<number>(0);

  // Group experiences by Year
  const groupedByYear = experiences.reduce<Record<string, ExperienceTimelineItem[]>>((acc, exp) => {
  ...
  const openModal = (exp: ExperienceTimelineItem) => {
    setSelectedExp(exp);
    setActiveGalleryImg(0);
  };

  // Reusable Single Timeline Card
  const renderCard = (exp: ExperienceTimelineItem, side: 'left' | 'right' = 'left', delay: number = 0) => {
